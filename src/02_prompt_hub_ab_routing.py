...
# Đổi thành tên của bạn — phải là duy nhất trong Hub của bạn
PROMPT_V1_NAME = "pham-tung-duong-rag-v1"
PROMPT_V2_NAME = "pham-tung-duong-rag-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
# Viết SYSTEM_V1 — phong cách ngắn gọn, trả lời 2-4 câu
...
# Viết SYSTEM_V2 — phong cách có cấu trúc, expert tone, 3-5 câu
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME,
            object=PROMPT_V1,
            description="V1 – câu trả lời ngắn gọn, trực tiếp",
        )
...
def main():
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        if version_tag == "v1":
            ...
            v2_count += 1
        log_line = f"[{i+1:02d}] [prompt-{version_tag}] {question}"
        routing_log.append(log_line)
        print(log_line)

    summary = f"Routing: V1={v1_count} câu | V2={v2_count} câu | Tổng={len(SAMPLE_QUESTIONS)}"
    print(f"\n📊 {summary}")
    evidence_path = Path(__file__).parent.parent / "evidence" / "02_ab_routing_log.txt"
    evidence_path.parent.mkdir(parents=True, exist_ok=True)
    evidence_path.write_text(
        "\n".join(routing_log + ["", summary]) + "\n",
        encoding="utf-8",
    )
    print(f"💾 Đã lưu routing log vào {evidence_path}")
